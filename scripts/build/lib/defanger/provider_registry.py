...
class ProviderRegistry:
    ...
    def _discover_and_register_providers(self):
        ...
        # IP2Location
        if "ip2location" in ENABLED_SERVICES:
            try:
                from .providers.ip2location import IP2LocationProvider
                provider = IP2LocationProvider(ENABLED_SERVICES["ip2location"])
                self._register_provider(provider)
            except ImportError as e:
                print(f"[Registry] Failed to load IP2Location provider: {e}")
        else:
            self.disabled_providers.append(("IP2Location", "IP2LOCATION_KEY", ["ip"]))
        
        # IPQualityScore
        if "ipqualityscore" in ENABLED_SERVICES:
            try:
                from .providers.ipqualityscore import IPQualityScoreProvider
                provider = IPQualityScoreProvider(ENABLED_SERVICES["ipqualityscore"])
                self._register_provider(provider)
            except ImportError as e:
                print(f"[Registry] Failed to load IPQualityScore provider: {e}")
        else:
            self.disabled_providers.append(("IPQualityScore", "IPQUALITYSCORE_KEY", ["ip"]))
